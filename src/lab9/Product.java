...
public abstract class Product implements Serializable, Iproduct {
    private int id;
    private String name;
    private int amount;
    ...
    public int comp1(Item a) {
        return this.name.compareTo(((Product)a).getName());
    }

    public int comp2(Item a) {
        if (this.amount < ((Product)a).getAmount()) {
            return -1;
        } else {
            return this.amount > ((Product)a).getAmount() ? 1 : 0;
        }
    }
